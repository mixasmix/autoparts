...
function getPart(event, part){
	...
	if(part.value){
		find_form=/*$('#body-container').html();*/'<div id="search"> <form style="" action="/" method="POST"> <h1>НАЙТИ АВТОЗАПЧАСТИ</h1> <input value="2888" style="" id="partNumber" name="part_number" placeholder="Введите номер запчасти" type="text"> <h6>Например, GXE10-0088644</h6> <!--<p>или</p><br><input type="text" name ="vin_number" placeholder="Введите VIN-номер вашего автомобиля" id="vinNumber"/><h6>Например, JN1WNYD21U0000001</h6>--><br> <input style="" id="searchPart" value="Найти" type="submit">&nbsp;</form> </div>';
		$('#body-container').html('');
...
function addpage(d){
	...
	if(!$('div').is('#partlist')){
		...
		$('#body-container').html(find_form+'<div id="partlist"></div>');
		$('#partlist').html('<div id="legend">	<p>Таким цветом <span class="colored"></span> выделены оригинальные детали по запрошенному номеру. <br>		Минимальный заказ по умолчанию - 1 шт. Если в поле "количесто" указано значение, отличное от 1, например 10 - то минимальный заказ позиции не может быть меньше 10 шт и должен быть кратным 10 - 10, 20, 30 и тд. Цена указана за одну единицу.	</p></div><div id="partlisttable"> <div class="div_header"> 	<div class="div_h col1">Артикул</div> 	<div class="div_h col2">Производитель</div> 	<div class="div_h col3">Описание</div> 	<div class="div_h col4">Информ.</div> <div class="div_h col10">Кол-во на складе</div>	<div class="div_h col5">Срок доставки</div> 	<div class="div_h col6">Вероятность отгрузки</div> <div class="div_h col7">Кол-во</div>	<div class="div_h col8">Цена</div>  <!--<div class="div_h col11"></div>-->	<div class="div_h col9">В корзину</div> </div></div> ');
		
		
		//получаем содержимое #body-container и переносим 
		/*$('#search').attr('style', 'width:100%;max-width:100%; position: relative; margin:0 auto;');*/
                $('#search').addClass('search_horizontal');
		/*$('#search').find('form').first().attr('style', 'height:150px;');*/
		/*$('#searchPart').attr('style', '');
		/*$('#searchPart').attr('style', '');*/
                $('#search').find('h1').first().remove();
		$('#partNumber').addClass('partNumber_search_horizontal');
		$('#searchPart').addClass('partNumber_search_horizontal');
...
$(window).scroll(function(){ if(isIntoView('#header'))
                              $('.scroll_top').show();
                            else
                                $('.scroll_top').hide();
});
...
/**
 * Функия дергает сервер для быстрого вывода данных с характеристикой товара
 * @returns {undefined}
 * 
 */
function pingInformationThisPartList(){
    var links=$('a.img_parts');
    links.each(function(indx, event){
        
        var div=$(event).parent().parent();
        var brand=div.children('div.col2').text();
        var part=div.children('div.col1').text();
        brand=brand.replace('/', ' ');
        brand=brand.replace('(', '');
        brand=brand.replace(')', '');
        //console.log(part+'/'+brand);
        $.get('/getimage/getinform/'+part+'/'+brand, function(data){});
    });
}