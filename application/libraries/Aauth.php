...
class Aauth {
	...
	public function remind_password($email){
		...
		if ($query->num_rows() > 0){
			...
			$this->CI->email->send();*/
			$headers = 'From: info@sts2.ru' . "\r\n";
                        mail(
                                $row->email,
                                $this->CI->lang->line('aauth_email_reset_subject'),
                                $this->CI->lang->line('aauth_email_reset_text') . site_url() . $this->config_vars['reset_password_link'] . $ver_code,
                                $headers
                            );
			return TRUE;
		}
	...
	public function is_totp_required(){
        ...
        public function is_login(){
            if($this->is_loggedin() and $this->get_user_id()!=4)
                return true;
           else
               return false;
        }
